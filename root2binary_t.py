...
def main():
   ...
   for event in tree:
      ...
      for pid,peta,pphi,ppt,pstat in zip(event.particle_id,
        ...
        # change electrin into muon and tauon and also change their particle id accordingly for different channels 
         if pid == 15 and tauon is None:
            logger.debug('e>> %5i %12.4f %12.4f %5.1f %7i ',pid,peta,pphi,ppt,pstat)
            tauon = {'id':pid,'eta':peta,'phi':pphi,'pt':ppt,'r':numpy.sqrt(peta*peta+pphi*pphi)}
         elif pid == -15 and antitauon is None:
            logger.debug('p>> %5i %12.4f %12.4f %5.1f %7i ',pid,peta,pphi,ppt,pstat)
            antitauon = {'id':pid,'eta':peta,'phi':pphi,'pt':ppt,'r':numpy.sqrt(peta*peta+pphi*pphi)}
      ...
      # list all the jets, eliminate them if they overlap with tauon/antitauon
      jets = []
      for jeta,jphi,jpt,jm in zip(event.tjet_eta,
         ...
         if ( numpy.fabs( jr - tauon['r']) > drlj and numpy.fabs( jr - antitauon['r']) ):
            logger.debug('j>> %5s %6.4f %6.4f %5.1f ',' ',jeta,jphi,jpt)
            jets.append({'eta':jeta,'phi':jphi,'pt':jpt,'r':numpy.sqrt(jeta*jeta+jphi*jphi)})
      
      #logger.debug('tauon: %s',tauon)
      #logger.debug('antitauon:%s',antitauon)
